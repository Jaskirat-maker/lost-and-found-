...
export function AdminDashboard() {
  ...
  const [lostId, setLostId] = useState<number | ''>('')
  const [foundId, setFoundId] = useState<number | ''>('')
  ...
  const load = async () => {
    setLoading(true)
    try {
      const res = await http.get<Item[]>('/api/admin/items', { params })
      setItems(res.data)
    } catch (e: unknown) {
                    ...
                    <td className="px-4 py-3">
                      <div className="flex flex-wrap gap-2">
                        ...
                        <button
                          type="button"
                          onClick={() => {
                            if (i.type === 'LOST') setLostId(i.id)
                            if (i.type === 'FOUND') setFoundId(i.id)
                            setMatchOpen(true)
                          }}
          ...
          <div className="grid gap-3 md:grid-cols-2">
            <div>
              ...
              <input
                inputMode="numeric"
                value={lostId}
                onChange={(e) => setLostId(e.target.value ? Number(e.target.value) : '')}
            ...
            <div>
              ...
              <input
                inputMode="numeric"
                value={foundId}
                onChange={(e) => setFoundId(e.target.value ? Number(e.target.value) : '')}
                className="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 outline-none focus:border-slate-400 dark:border-slate-800 dark:bg-slate-950"
                placeholder="e.g. 19"
              />
            </div>
